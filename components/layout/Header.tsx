...
export default function Header() {
  ...
  return (
    ...
    >
      <div className="w-full h-[65px] sm:h-[80px] md:h-[90px] px-[16px] sm:px-6 md:px-8 lg:px-[118px] flex items-center justify-between max-sm:px-[16px] max-sm:h-[70px] overflow-visible">
        ...
        <div className="flex items-center gap-3 sm:gap-4 md:gap-5 ml-auto relative">
          ...
          <div className="relative">
            <button
              ...
              className="px-3 py-1 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all text-[#132219] font-semibold"
            >
              {language}
            </button>
              ...
              <div className="absolute right-0 mt-2 w-[140px] bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden animate-fadeIn max-sm:right-auto max-sm:left-0 max-sm:mt-[10px]">
                {["DE", "FR", "IT", "EN"].map((lang) => (
                  <button
                    ...
                    className="w-full px-4 py-3 text-[#132219] hover:bg-gray-50 text-[15px] text-left"
                  >
                    {lang}
                  </button>
                ))}
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-md px-6 py-6 flex flex-col gap-4 animate-fadeIn max-sm:px-[24px] max-sm:py-[18px] max-sm:gap-[14px]">
          <Link
            href="/partner"
